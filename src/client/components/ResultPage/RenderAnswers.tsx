...
const RenderAnswers = ({ survey, resultData }: { survey: Survey; resultData: FormValues }) => {
                ...
                <Box sx={resultStyles.card}>
                  <Typography>
                    {currentQuestion.id === 8 && resultData[4] === 'multilateral' ? (
                      <b>{t('questions:additionalPartnerOrganisationCountryQuestion')}</b>
                    ) : currentQuestion.id === 6 && resultData[4] === 'multilateral' ? (
                      <b>{t('questions:additionalPartnerOrganisationTypeQuestion')}</b>
                    ) : (
                      ...
                      <Box>
                        {answers[childQuestion.id] && (
                          <Box sx={resultStyles.answerBox}>
                            <Typography>
                              <b>{childQuestion.title[language as keyof Locales]}</b>
                            </Typography>
                            <Typography>
                              {childQuestion.id === 26
